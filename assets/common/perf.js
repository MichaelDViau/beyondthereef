  ...
  */
  ...
  function setupGlobalPromo() {
    ...
    if (!doc.getElementById('promoFloatStyles')) {
      ...
      style.textContent = '' +
        '.promo-float{position:fixed;left:20px;bottom:16px;z-index:400;width:min(300px,calc(100vw - 40px));background:rgba(255,140,0,1);border:1px solid rgba(255,255,255,.35);border-radius:14px;filter:drop-shadow(0 4px 12px rgba(139,69,0,.25));padding:14px 38px 14px 14px;overflow:hidden;text-decoration:none;clip-path:inset(0 0 0 0 round 14px);transition:clip-path .32s cubic-bezier(0,0,0.2,1)}' +
        '.promo-main{opacity:1;transition:opacity .18s ease .28s}' +
        '.promo-close{position:absolute;top:8px;right:8px;width:22px;height:22px;border:0;border-radius:50%;background:rgba(255,255,255,.25);color:#fff;font-size:14px;font-weight:700;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;opacity:1;transition:opacity .18s ease .28s}' +
        ...
        '.promo-float.is-collapsed{width:52px;height:52px;padding:0;border-radius:50%;clip-path:none;cursor:pointer;transition:width .24s cubic-bezier(0.4,0,1,1),height .24s cubic-bezier(0.4,0,1,1),padding .24s cubic-bezier(0.4,0,1,1),border-radius .24s cubic-bezier(0.4,0,1,1)}' +
        ...
        '.promo-float.is-collapsed:hover{filter:drop-shadow(0 6px 18px rgba(139,69,0,.4)) brightness(1.08)}' +
        '@media(max-width:767px){.promo-float{left:10px;bottom:16px;width:min(270px,calc(100vw - 20px));padding:12px 34px 12px 12px}.promo-text{font-size:12px}.promo-link{font-size:11px;padding:7px 10px}}';
